...
//        Hello World! Yazının sitede oldugunu test et
        //1. Fluent wait icin ir obje olusturmak gerekiyor
        Wait<WebDriver> wait = new FluentWait<>(driver)
                .withTimeout(Duration.ofSeconds(30)) //MAX BEKLEME SURESI
                .pollingEvery(Duration.ofSeconds(3)) //DENEME ARALIKLARI
                .withMessage("IGNORE EXCEPTION") //OPSIYONEL OLARAK-ISTERSEK MESAJ YAZABILIRIZ(ZORUNLU DEGIL)
                .ignoring(NoSuchMethodException.class); //(ZORUNLU DEGIL) EXCEPTION'U HANDLE ET(UMURSAMA)

        //2.wait objesi kullanarak bekleme problemini coz.
